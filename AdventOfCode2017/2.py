...
def partOne():
    checksum = 0
    for row in rowsInString(sheet):
        numbers = numbersInRow(row)
        checksum = checksum + max(numbers) - min(numbers)
    print(checksum)

def rowsInString(string):
    return string.split("\n")
...
def partTwo():
    ...
    for row in rowsInString(sheet):
        numbers = numbersInRow(row)
        a,b = findABSoADividesB(numbers)
        total += b//a
    print(total)

def findABSoADividesB(numbers):
...
sheet = '''1208	412	743	57	1097	53	71	1029	719	133	258	69	1104	373	367	365
